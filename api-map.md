...
| `saveRequiredTubeRule` | Yes | `POST` | 20s | 1 | `[testName, sst, edta, fluoride, urine, consent, notes]` | `{ status: 'success', message: '...', rule: {...} }` | `saveTubeChecklistRuleFromModal` (`app.js`) | Saves or updates a test rule in the "Required Tube Checklist Helper" sheet directly from the frontend modal |
